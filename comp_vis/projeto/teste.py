...
from quaternion_euler_utility import quat_euler, euler_quat, quat_rot_mat
...
class MyApp(ShowBase):
    def __init__(self):
        ...
        #LOAD TRAINED POLICY
        try:
            self.policy.load_state_dict(torch.load('PPO_continuous_solved_drone.pth',map_location=device))
        ...
        self.scene = self.loader.loadModel(mydir + "/city.egg")
        ...
        # Load the skybox
        self.skybox = self.loader.loadModel(mydir + "/skybox.egg")
        ...
        self.quad = self.loader.loadModel(mydir + '/quad_2.egg')
        self.quad.reparentTo(self.render)
        self.prop_1 = self.loader.loadModel(mydir + '/prop.egg')
        self.prop_1.setPos(-0.26,0,0)
        self.prop_1.reparentTo(self.quad)
        self.prop_2 = self.loader.loadModel(mydir + '/prop.egg')
        self.prop_2.setPos(0,0.26,0)
        self.prop_2.reparentTo(self.quad)
        self.prop_3 = self.loader.loadModel(mydir + '/prop.egg')
        self.prop_3.setPos(0.26,0,0)
        self.prop_3.reparentTo(self.quad)
        self.prop_4 = self.loader.loadModel(mydir + '/prop.egg')
        self.prop_4.setPos(0,-0.26,0)
        self.prop_4.reparentTo(self.quad)
        
        # Load the checkerboard actor
        self.checker = self.loader.loadModel(mydir+ '/checkerboard.egg')
    ...
    def calibrate(self, task):
        if task.frame == 0:
            ...
            try: 
                npzfile = np.load('Calibracao.npz')
